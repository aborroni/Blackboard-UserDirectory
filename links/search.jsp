...
else if(userPortalRoleId.equals(facultyPortalRole.getId()) || userPortalRoleId.equals(staffPortalRole.getId()))
{
    ...
    // check if some one has appointments
    boolean has_appts = true;
    try {
        URL signupPage = new URL("http://octet1.csr.oberlin.edu/octet/Bb/AppointmentsEX/check.php?name=" + user.getUserName());
        BufferedReader in = new BufferedReader(new InputStreamReader(signupPage.openStream()));
    ...
    // output different things depends of appointment availablilty
    if (has_appts){
